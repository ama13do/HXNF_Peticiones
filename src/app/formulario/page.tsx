...
export default function FormularioPage() {
  ...
  const [modoMensaje, setModoMensaje] = useState<"plantilla" | "personalizado">(
    "plantilla",
  );
  ...
  const buscarLegisladores = useCallback(async () => {
    ...
    try {
      let dipQuery = supabase
        ...
        .select("*")
        .or(
          `Entidad.ilike.%${estadoDetectado}%,Estado_Eleccion.ilike.%${estadoDetectado}%`,
        );
      ...
      const [dipRes, senRes] = await Promise.all([
        ...
        supabase
          ...
          .select("*")
          .or(
            `Estado.ilike.%${estadoDetectado}%,estadoOrigen.ilike.%${estadoDetectado}%`,
          ),
      ]);
  ...
  const seleccionarTodos = () => {
    const keys: string[] = [];
    if (filtroTipo !== "senadores")
      diputadosFiltrados.forEach((d) => keys.push(`dip-${d.Id}`));
    if (filtroTipo !== "diputados")
      senadoresFiltrados.forEach((s) => keys.push(`sen-${s.id}`));
  ...
  const validar = (): boolean => {
    ...
    if (!nombre.trim() || !apellidoPaterno.trim())
      faltantes.push("Nombre y Apellido Paterno");
    if (!correo.trim() || !correo.includes("@"))
      faltantes.push("Correo electrónico válido");
    if (telefono.trim().length !== 10)
      faltantes.push("Teléfono válido a 10 dígitos");
    if (!cp.trim() || cp.length < 5)
      faltantes.push("Código Postal a 5 dígitos");
    if (!mensajePersonalizado.trim())
      faltantes.push("Seleccionar o escribir una petición");
  ...
  const guardarEnDB = async (
    ...
    try {
      ...
      console.log("Emails:", emailsDestino);
      console.log("Legisladores:", legisladoresIds);

      // ─────────────────────────────────────────────
      // 1. Guardar colaborador
      // ─────────────────────────────────────────────

      console.log("INSERTANDO COLABORADOR...");
        ...
        .upsert(
          {
            ...
            codigo_postal: cp,
            distrito_electoral: distritoElectoral
              ? parseInt(distritoElectoral)
              : null,
      ...
      console.log("COLAB ERROR:", colabError);

      if (colabError) {
        console.error("ERROR AL GUARDAR COLABORADOR:", colabError);
        alert(`Error colaborador:\n${JSON.stringify(colabError, null, 2)}`);
        return false;
      }

      if (!colabData?.id) {
        console.error("NO SE OBTUVO ID DE COLABORADOR");
        alert("No se obtuvo el ID del colaborador.");
        return false;
      }
      ...
      console.log("INSERTANDO PETICION...");
      ...
      if (petError) {
        console.error("ERROR AL GUARDAR PETICION:", petError);
        alert(`Error petición:\n${JSON.stringify(petError, null, 2)}`);
        return false;
      }

      if (!petData?.id) {
        console.error("NO SE OBTUVO ID DE PETICION");
        alert("No se obtuvo el ID de la petición.");
        return false;
      }

      // ─────────────────────────────────────────────
      // 3. Guardar destinatarios
      // ─────────────────────────────────────────────

      if (legisladoresIds.length > 0) {
        console.log("INSERTANDO DESTINATARIOS...");

        const destinatarios = legisladoresIds.map((l) => ({
        ...
        const { data: destData, error: destError } = await supabase
          .from("destinatarios_peticion")
          .insert(destinatarios)
          .select();

        console.log("DEST DATA:", destData);
        console.log("DEST ERROR:", destError);

        if (destError) {
          console.error("ERROR AL GUARDAR DESTINATARIOS:", destError);
          alert(`Error destinatarios:\n${JSON.stringify(destError, null, 2)}`);
          return false;
        }
      }

      console.log("GUARDADO COMPLETADO CORRECTAMENTE");
      return true;
    } catch (err) {
  ...
  const enviarSeleccionados = async () => {
    ...
    seleccionados.forEach((key) => {
      if (key.startsWith("dip-")) {
        const d = diputados.find(
          (x) => x.Id === parseInt(key.replace("dip-", "")),
        );
        if (d) {
          const e = getEmailDip(d);
          if (e) emails.push(e);
          ids.push({ id: d.Id, tipo: "diputado" });
        }
      } else {
        const s = senadores.find(
          (x) => x.id === parseInt(key.replace("sen-", "")),
        );
        if (s) {
          const e = getEmailSen(s);
          if (e) emails.push(e);
          ids.push({ id: s.id, tipo: "senador" });
        }
      }
    });
  ...
  const CardDip = ({ d }: { d: Diputado }) => {
    ...
    const initials = (d.Nombre_Completo ?? "?")
      .split(" ")
      .slice(0, 2)
      .map((w) => w[0])
      .join("")
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-white/40 text-xs truncate">
            {[
              d.Partido,
              d.Numero_Distrito && `Dto. ${d.Numero_Distrito}`,
              d.Ciudad_Eleccion,
            ]
              .filter(Boolean)
              .join(" · ")}
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 pb-32 lg:pb-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/50 max-w-lg mx-auto text-sm">
            Completa tus datos, elige el mensaje y envía directo desde tu
            correo.
          </p>
        </div>
          ...
          <div className="flex-1 space-y-5 w-full">
            ...
            <div className="border border-white/10 rounded-2xl overflow-hidden">
              <div className="bg-white/5 px-5 py-4 border-b border-white/10">
                ...
                <p className="text-white/40 text-xs">
                  Personalizan tu carta · no se almacenan en servidores externos
                </p>
              </div>
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input
                    type="text"
                    value={distritoElectoral}
                    onChange={(e) => setDistritoElectoral(e.target.value)}
            ...
            <div className="border border-white/10 rounded-2xl overflow-hidden">
              <div className="bg-white/5 px-5 py-4 border-b border-white/10">
                ...
                <p className="text-white/40 text-xs">
                  Selecciona un tema o escribe tu propio mensaje
                </p>
              </div>
              ...
              <div className="p-5 space-y-4">
                        ...
                        {plantillas.map((p) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setMensajePersonalizado(p.cuerpo);
                            }}
                          ...
                          >
                            ...
                            <div className="text-xs mt-0.5 opacity-60 line-clamp-1">
                              {p.asunto}
                            </div>
                          </button>
                        ))}
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div
                    ...
                    className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors cursor-pointer ${
                      aceptaPrivacidad
                        ? "bg-hxnf-green border-hxnf-green"
                  ...
                  <span className="text-white/50 text-xs leading-relaxed">
                    ...
                    <a
                      href="/#privacidad"
                      className="text-hxnf-green underline"
                    >
                      aviso de privacidad
                    </a>
                    . Entiendo que se registrará mi petición y que el correo se
                    enviará desde mi propia app.
                  </span>
                </label>
                ...
                <label className="flex items-start gap-3 cursor-pointer group mt-4">
                  ...
                  <span className="text-white/50 text-xs leading-relaxed mt-0.5">
                    ¿Te gustaría recibir más información para sumarte a cambiar
                    las cosas?
                  </span>
                </label>
              </div>
              ...
              {!hayLegisladores ? (
                <div className="p-10 text-center">
                  ...
                  <p className="text-white/25 text-xs mt-1">
                    Detectamos tu estado y cargamos automáticamente
                  </p>
                </div>
              ) : (
                <>
                  <div className="px-5 py-3 border-b border-white/5 space-y-3">
                    <div className="flex gap-2 flex-wrap">
                      {(
                        ["todos", "diputados", "senadores"] as FiltroTipo[]
                      ).map((t) => (
                        <button
                          key={t}
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      <button
                        ...
                        className="text-hxnf-green hover:underline"
                      >
                        Seleccionar todos
                      </button>
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {diputadosFiltrados.map((d) => (
                              <CardDip key={d.Id} d={d} />
                            ))}
                          </div>
                        </div>
                      )}

                    {filtroTipo !== "diputados" &&
                      senadoresFiltrados.length > 0 && (
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs bg-white/10 text-white/40 px-2 py-0.5 rounded-full">
                              {senadoresFiltrados.length}
                            </span>
                          </div>
                          <div className="space-y-2">
                            {senadoresFiltrados.map((s) => (
                              <CardSen key={s.id} s={s} />
                            ))}
                          </div>
                        </div>
                      )}
          ...
          <div className="hidden lg:flex lg:w-72 flex-col gap-4 sticky top-20">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex gap-3 items-start">
                    <span className="w-5 h-5 rounded-full bg-black border-2 border-hxnf-green text-hxnf-green text-xs font-bold flex items-center justify-center flex-shrink-0">
                      {i + 1}
                    </span>
            ...
            <div className="border border-white/10 bg-white/5 rounded-2xl p-4">
              <p className="text-white/40 text-xs leading-relaxed">
                ...
                . Los correos van directo desde tu app de correos. Copia oculta
                a <span className="text-hxnf-green">hxnf@practica.lat</span>.
              </p>
            </div>
          </div>
        </div>
